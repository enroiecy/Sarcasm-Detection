...
pred_ids_str = '\n'.join(str(index) for index in pred_ids)
with open('../data/' + dataset + '.pred.index', 'w') as f:
    f.write(pred_ids_str)

#-----------------------------------------------------------------------------
ids = train_ids + test_ids + pred_ids
print(ids)
print(len(ids))
...
# px: feature vectors of predict docs, no initial features
...
for i in range(pred_size):
    doc_vec = np.array([0.0 for k in range(word_embeddings_dim)])
    doc_words = shuffle_doc_words_list[i + train_size + test_size]
    words = doc_words.split()
    doc_len = len(words)
    for word in words:
        if word in word_vector_map:
    ...
    for j in range(word_embeddings_dim):
        ...
        col_px.append(j)
        # np.random.uniform(-0.25, 0.25)
        data_px.append(doc_vec[j] / doc_len)  # doc_vec[j] / doc_len

# tx = sp.csr_matrix((test_size, word_embeddings_dim), dtype=np.float32)
px = sp.csr_matrix((data_px, (row_px, col_px)),
                   shape=(pred_size, word_embeddings_dim))
py = []
for i in range(pred_size):
    doc_meta = shuffle_doc_name_list[i + train_size + test_size]
    temp = doc_meta.split('\t')
    label = temp[2]
    one_hot = [0 for l in range(len(label_list))]
    label_index = label_list.index(label)
...
print(x.shape, y.shape, tx.shape, ty.shape, px.shape, py.shape, allx.shape, ally.shape)
...
with open("../data/ind.{}.px".format(dataset), 'wb') as f:
    pkl.dump(px, f)

with open("../data/ind.{}.ty".format(dataset), 'wb') as f:
    pkl.dump(ty, f)
    
with open("../data/ind.{}.py".format(dataset), 'wb') as f:
    pkl.dump(py, f)  
